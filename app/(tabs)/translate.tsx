...
export default function TranslateScreen() {
  ...
  useEffect(() => {
    ...
    const newPairs = toQAPairs(chatMessages, {
      fallbackError: chatError,
      context: chatContext,
    });

    // 既存のfollowUpQAsを保持してマージ
    const mergedPairs = newPairs.map(newPair => {
      const existingPair = qaPairs.find(p => p.q === newPair.q);
      if (existingPair?.followUpQAs && existingPair.followUpQAs.length > 0) {
        logger.debug('[Translate] Preserving followUpQAs for question:', {
          question: newPair.q.substring(0, 50),
          followUpCount: existingPair.followUpQAs.length,
        });
