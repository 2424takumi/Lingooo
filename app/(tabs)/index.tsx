...
export default function HomeScreen() {
  ...
  // クォータ制限からのアップグレード遷移: AsyncStorageフラグでサブスクモーダルを自動表示
  useFocusEffect(
    useCallback(() => {
      AsyncStorage.getItem('@lingooo:show_subscription').then((val) => {
        if (val === 'true') {
          AsyncStorage.removeItem('@lingooo:show_subscription');
